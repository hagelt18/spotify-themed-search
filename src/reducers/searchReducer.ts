  ...
  switch (action.type) {
    ...
    case SET_ITEM_LISTENED: {
      ...
      if (currentResults && item?.track) {
        const actualItemIndex = currentResults?.tracks.findIndex(
          (t) => t.id === item.track?.id,
        );
        if (actualItemIndex) {
          const newTrackList: Track[] = [
            ...(currentResults?.tracks || []).map((t) => ({ ...t })),
          ];
          newTrackList[actualItemIndex].listened = true;
          newResults = { ...currentResults, tracks: newTrackList };
        } else {
          console.warn(
            "Unable to find selected item by track id:",
            item.track?.id,
          );
        }
      }
      if (currentResults && item?.album) {
        const actualItemIndex = currentResults?.albums.findIndex(
          (t) => t.id === item.album?.id,
        );
        const newItemList = [
          ...(currentResults?.albums || []).map((a) => ({ ...a })),
        ];
