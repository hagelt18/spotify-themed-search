...
export const SearchActionBar = () => {
  ...
  const [showNewSearchCriteria, setShowNewSearchCriteria] =
    useState<boolean>(false);
  const [newSearchTheme, setNewSearchTheme] = useState<Theme>(
    themes.themeList.find((t) => t.id === themes.defaultThemeId) as Theme,
  );
  const [newSearchYear, setNewSearchYear] = useState<string>(
    new Date().getFullYear().toString(),
  );

  const { results } = useSelector<AppState>(
    (store) => store.search,
  ) as SearchState;
  ...
  const onLoadDataFromFile = (e: any) => {
    ...
    fileReader.onload = function (fileLoadedEvent: any) {
      ...
      const defaultTheme = themes.themeList.find(
        (t) => t.id === themes.defaultThemeId,
      ) as Theme;
      const loadedResultTheme =
        themes.themeList.find((t) => t.id === loadedResults.themeId) ||
        defaultTheme;
      const loadedResultsSearchYear =
        loadedResults.searches?.[0]?.searchYear || newSearchYear;
      const loadedSearchCriteria = getSearchCriteria(
        loadedResultTheme,
        loadedResultsSearchYear,
      );
  ...
  const handleThemeChange = (event: any) => {
    ...
    const selectedTheme = themes.themeList.find(
    ...
    <div>
      ...
      >
        <Button
          ...
          size="sm"
          onClick={() => {
            setShowNewSearchCriteria(!showNewSearchCriteria);
          }}
        >
          New
        </Button>
        ...
        <>
          ...
          {saveResultsUrl && (
            <a
              download={"spotify-search-results.json"}
              href={saveResultsUrl}
              style={{ display: "none" }}
              ref={saveFileInput}
            >
      ...
      <div
        style={{
          display: showNewSearchCriteria ? "block" : "none",
          marginTop: "10px",
          padding: "10px",
      ...
      >
        <Form>
          <Row className="mb-3">
            <Form.Group
              as={Col}
              controlId="searchButton"
              style={{ maxWidth: "fit-content", height: "100%" }}
            >
              <Button
                name="searchButton"
