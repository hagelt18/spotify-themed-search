...
export const SearchSummaries = () => {
  ...
  const onLoadRestClicked = async (searchDetails: SearchDetails) => {
    ...
    const newResults = await loadRest(
      results || initializeFinalResults(),
      searchDetails,
    );
    dispatch(
      searchComplete({
  ...
  const groupedSearches = useMemo<GroupedSearches | undefined>(() => {
    return results?.searches?.reduce(
      (groups, resultSet) => {
        const existingGroup = groups.groupList.find(
          (g) => g.searchTerm === resultSet.searchTerm,
        );
        if (existingGroup) {
  ...
  return (
    <div className="SearchSummaries">
      ...
      {groupedSearches?.groupList?.map((searchGroup) => (
        ...
        <div key={searchGroup?.searchTerm}>
          <div>
            <strong>{`${searchGroup.searchTerm}`}</strong>
          </div>
            ...
            <div key={search.key} className="SearchSummaryDetailLine">
              ...
              <div>
                {search.totalCount == null
                  ? "Not loaded"
                  : `${search.totalLoaded} of ${search.totalCount}`}
              </div>
              {(search.totalCount == null ||
                ...
                <div>
                  <button
                    ...
                    className="btn-icon"
                    onClick={() => onLoadMoreClicked(search)}
                    title="Load next"
                  >
                    <FaArrowRight />
                  </button>
                  <button
                    className="btn-icon"
                    onClick={() => onLoadRestClicked(search)}
                    title="Load rest"
                  >
                    <FaDownload />
                  </button>
