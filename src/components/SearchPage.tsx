...
const SearchPage = (props: SearchPageProps) => {
  ...
  const { theme, results, loading, error, selectedItem } =
    useSelector<AppState>((store) => store.search) as SearchState;
  // On mount
  useEffect(() => {
    if (!isValidSession()) {
      navigate("/");
          ...
          {results && (
            <>
              <div style={{ margin: "1em 0px" }}>
                Total Tracks: {results.tracks?.length} - Total Albums:{" "}
              ...
              <FormCheck
                label={`Hide listened (Tracks: ${tracksListenedCount}), Albums: ${albumsListenedCount})`}
                value={!hideListenedToItems ? 1 : 0}
                onChange={(e) => {
                  setHideListenedToItems(e.target.checked);
                }}
              />
            </>
          )}
