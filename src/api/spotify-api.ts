...
export const executeInitialSearch = async (
  searchCriteria: SearchCriteria,
): Promise<NewResults[]> => {
  return executeAllSearches(searchCriteria);
};

export const loadRest = async (
  currentResults: FinalResults,
  resultSet: SearchDetails,
) => {
...
export const loadMore = async (
  ...
  if (search.next && !passedMaxOffset(search.next)) {
    ...
    apiResult = await executeOneSearchByCriteria(
      search.searchType,
      search.searchTerm,
      search.searchYear,
    );
  ...
  const next =
    search.searchType === "track"
      ? apiResult?.tracks?.next
      : apiResult?.albums?.next;
  const totalCount = apiResult?.tracks?.total || apiResult?.albums?.total || 0;
  const totalLoaded =
    apiResult?.tracks?.items?.length || apiResult?.albums?.items?.length || 0;
...
export const addAllSearchCriteriaToResults = (
  searchCriteria: SearchCriteria,
  currentResults: FinalResults,
) => {
  ...
  for (let searchType of searchTypes) {
    for (let searchTerm of searchTerms) {
      ...
      const existingSearch = currentResults?.searches.find(
        (x) => x.key === searchKey,
      );
      if (!existingSearch) {
...
export const executeAllSearches = async (
  searchCriteria: SearchCriteria,
): Promise<NewResults[]> => {
  ...
  for (let searchType of searchTypes) {
    for (let searchTerm of searchTerms) {
      ...
      // } else {
      const apiResult = await executeOneSearchByCriteria(
        searchType,
        searchTerm,
        searchYear,
      );
      newSearchResults.unshift({
        searchDetails: {
          ...
          totalLoaded:
            apiResult?.tracks?.items?.length ||
            apiResult?.albums?.items?.length ||
            0,
...
export const executeOneSearchByCriteria = async (
  searchType: SearchType,
  searchTerm: string,
  searchYear: string,
): Promise<SpotifyApiSearchResult> => {
  ...
  const API_URL = `https://api.spotify.com/v1/search?query=${encodeURIComponent(
    query,
  )}&type=${searchType}&limit=${SEARCH_LIMIT}`;
...
export const addToFinalResults = (
  newSearchResults: NewResults[],
  finalResults: FinalResults,
): FinalResults => {
  (newSearchResults || []).forEach((resultSet) => {
    const existingSearchIndex = finalResults?.searches.findIndex(
      (x) => x.key === resultSet.searchDetails.key,
    );
    if (existingSearchIndex >= 0) {
      finalResults.searches[existingSearchIndex] = {
        ...finalResults.searches[existingSearchIndex],
        totalLoaded:
          finalResults.searches[existingSearchIndex].totalLoaded +
          resultSet.searchDetails.totalLoaded,
    ...
    (resultSet?.spotifyResults || []).forEach((result) => {
      if (resultSet.searchDetails.searchType === "track") {
        (result?.tracks?.items ?? []).forEach((track) => {
          ...
          if (
            !finalResults.tracks.find(
              (t) =>
                (t.name === trackInfo.name &&
                  t.artists === trackInfo.artists) ||
                t.id === trackInfo.id,
            )
          ) {
        ...
        (result?.albums?.items ?? []).forEach((album) => {
          ...
          if (
            ...
            !finalResults.albums.find(
              (a) =>
                (a.name === albumInfo.name &&
                  a.artists === albumInfo.artists) ||
                a.id === albumInfo.id,
            )
          ) {
...
const getSearchKey = (searchType: SearchType, searchTerm: string) =>
